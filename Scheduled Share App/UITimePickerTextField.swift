...
class UITimePickerTextField: UITextField {
    ...
    @objc func handleFocused(notification: NSNotification!) {
        ...
        // UIToolbar の設定
        // 決定バーの生成
        let toolbar = UIToolbar(frame: CGRect(x: 390, y: 0, width: frame.size.width, height: 35))
        let cancel = UIBarButtonItem(barButtonSystemItem: .cancel, target: self, action: #selector(cancelButton))
        let doneItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(done))
        toolbar.setItems([cancel, doneItem], animated: true)
